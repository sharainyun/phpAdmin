    ...
    <div class="filter-container">
      <el-form :inline="true" :model="optionParams" class="demo-form-inline" ref="params">
        ...
        <el-form-item label="状态" prop="status">
          <el-select v-model="optionParams.status" placeholder="请选择审批类型">
            <el-option label="待审批" value="1"></el-option>
            <el-option label="拟通过" value="2"></el-option>
      ...
      <div class="tablelayer">
        ...
        >
          ...
          <el-table-column align="center" label="操作" fixed="right" width="330">
            <template slot-scope="scope">
              <span v-if="scope.row.status==='1'&&userType1&&shenpiList.oprater!=scope.row.username">
                <el-button
                type="primary"
                @click="dialogVisible1 = true;shenpiList.status = 2;shenpiList.Id = scope.row.Id;shenpiList.reason = '';"
                >拟通过</el-button>
              <el-button type="danger" @click="dialogVisible1 = true;shenpiList.status = 3;shenpiList.Id = scope.row.Id;shenpiList.reason = '';">拒绝</el-button>
              </span>
              <span v-if="scope.row.status==='2'&&userType3&&shenpiList.oprater!=scope.row.username">
                <el-button
                type="primary"
                @click="dialogVisible1 = true;shenpiList.status = 4;shenpiList.Id = scope.row.Id;shenpiList.reason = '';"
              >审批通过</el-button>
              <el-button type="danger" @click="dialogVisible1 = true;shenpiList.status = 3;shenpiList.Id = scope.row.Id;shenpiList.reason = '';">拒绝</el-button>
              </span>
              <span v-if="userType3&&shenpiList.oprater!=scope.row.username">
                <el-button type="danger" icon="el-icon-delete" @click="deleteRows (scope.row.Id)">删除</el-button>
              </span>
            </template>
          </el-table-column>
        </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      userType1:Cookies.get('userType')==='1'||Cookies.get('userType')===1,
  ...
  created() {
    ...
    // console.log(typeof(Cookies.get('userType')));
    // console.log(1&&this.userType1);
