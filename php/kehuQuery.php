<?php
	...
	$res=mysql_query("select * from kehu WHERE {$tj1} AND {$tj2}");//查询kehu表里所有数据 
	$total = mysql_num_rows(mysql_query("select * from kehu WHERE {$tj1} AND {$tj2}"));//查询所有符合条件的数据（不包括分页条件） 
