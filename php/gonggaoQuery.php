<?php
    ...
    // $total = mysql_query("select * from gonggao WHERE {$tj1} AND {$tj2}");
    $res = mysql_query("select * from gonggao  WHERE {$tj1} AND {$tj2} limit $limit1,$limit2");//查询表里所有符合条件的数据  
    $total = mysql_num_rows(mysql_query("select * from gonggao WHERE {$tj1} AND {$tj2}"));//查询所有符合条件的数据（不包括分页条件）
